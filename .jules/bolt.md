...
## 2026-07-14 - Unthrottled State Updates on Heavy Components
**Learning:** Found a critical performance bottleneck in `AnalyticsDashboard.tsx` where high-frequency events (up to 10 emits/sec from Tauri backend) updated the component state immediately. Because the state contained large arrays and the component itself is heavy (multiple Recharts), updating the state 10 times a second crippled the main thread and caused high CPU usage. Also, mutating outer variables inside React state setters (like `packetCountInSec = 0`) can cause bugs if React double-invokes the setter in Strict Mode.
**Action:** Always buffer and throttle React state updates for high-frequency events (like IPC listeners) when dealing with heavy dashboard components. Ensure buffers are safely copied and external mutable state is reset *outside* the `setState` callbacks.
