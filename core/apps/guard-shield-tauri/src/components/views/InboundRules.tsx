...
import { ArrowLeftCircle, Trash2, Plus, Power, PowerOff } from "lucide-react";
...
export default function InboundRules() {
  ...
  const loadRules = async () => {
    try {
      ...
      setRules(fetched.filter(r => r.direction === "Inbound" || r.direction === "Both"));
    } catch(e) {
          ...
          <div className="flex-1 relative overflow-hidden bg-muted/10 flex flex-col p-6">
            <div className="flex items-center justify-between mb-6">
              <div className="flex items-center gap-2">
                <ArrowLeftCircle className="size-6 text-primary" />
                <h1 className="text-2xl font-bold tracking-tight">Inbound Rules</h1>
              </div>
        ...
        <div className="space-y-4 pr-4">
          {rules.length === 0 ? (
            <div className="flex flex-col items-center justify-center h-64 text-muted-foreground bg-muted/20 rounded-lg border border-dashed">
              <ArrowLeftCircle className="size-12 mb-4 opacity-20" />
